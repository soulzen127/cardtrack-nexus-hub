...
export const LoginForm = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Mock login - in a real app you would use proper authentication
      console.log("Logging in with:", { email, password });
      
      // For demo purposes, any email/password combination works
      localStorage.setItem("authenticated", "true");
      
      // If system is not initialized, set this user as normal user
      // First time users will need to setup themselves as admin in settings
      if (!isSystemInitialized) {
        localStorage.setItem("user_role", "viewer");
        localStorage.setItem("user_name", email.split("@")[0]);
        toast.success(t("loginSuccessFirstTime"));
      } else {
        // Mock different roles based on email prefix for testing
  ...
  const handleSocialLogin = (provider: string) => {
    setIsLoading(true);
    // In a real app, this would integrate with the respective auth provider
    console.log(`Logging in with ${provider}`);
    
    setTimeout(() => {
      localStorage.setItem("authenticated", "true");
      
      // If system is not initialized, set this user as standard user
      if (!isSystemInitialized) {
        localStorage.setItem("user_role", "viewer");
      } else {
        localStorage.setItem("user_role", "viewer"); // Default role for social login
      }
